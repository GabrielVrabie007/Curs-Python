...
Sistemul va conține o clasă `Sistem` care va conține o listă de utilizatori și metode pentru a adăuga utilizatori noi, 
a afișa utilizatorii existenți și a verifica nivelul de acces al unui utilizator.
La sistem va avea acces doar Utilizatorii Admin așa că trebuie să ne asigurăm că aceștia vor avea metode pentru a adăuga,
 a modificare și a șterge datele private ale sistemului.
...
# CODUL TĂU VINE MAI JOS:
class user:
    def __init__(self, _nume,__nivel_acces="Default"):
        self.__nivel_acces = __nivel_acces
        self._nume = _nume

    def get_nume(self):
        return self.__nume

    def set_nume(self, __nume):
        self.__nume = __nume

    def get_nivel_acces(self):
        return self.__nivel_acces

    def set_nivel_acces(self, __nivel_acces):
        self.__nivel_acces = __nivel_acces
...
class Sistem:
    ...
    def adauga_utilizator(self, utilizator):
        user_id = len(self.__utilizatori) + 1
        self.__utilizatori[user_id] = utilizator
    ...
    def modifica_nume_utilizator(self, user_id, new_name):
